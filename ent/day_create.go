...
func (dc *DayCreate) SetCreatedAt(t time.Time) *DayCreate {
	dc.mutation.SetCreatedAt(t)
	return dc
}

// SetNillableCreatedAt sets the "created_at" field if the given value is not nil.
func (dc *DayCreate) SetNillableCreatedAt(t *time.Time) *DayCreate {
	if t != nil {
		dc.SetCreatedAt(*t)
	}
	return dc
...
func (dc *DayCreate) SetTxt(s string) *DayCreate {
	dc.mutation.SetTxt(s)
	return dc
}

// AddPlanIDs adds the "plan" edge to the Plan entity by IDs.
func (dc *DayCreate) AddPlanIDs(ids ...int) *DayCreate {
	dc.mutation.AddPlanIDs(ids...)
	return dc
}

// AddPlan adds the "plan" edges to the Plan entity.
func (dc *DayCreate) AddPlan(p ...*Plan) *DayCreate {
	ids := make([]int, len(p))
	for i := range p {
		ids[i] = p[i].ID
	}
...
func (dc *DayCreate) check() error {
	if _, ok := dc.mutation.CreatedAt(); !ok {
		return &ValidationError{Name: "created_at", err: errors.New(`ent: missing required field "Day.created_at"`)}
	}
	if _, ok := dc.mutation.Date(); !ok {
		return &ValidationError{Name: "date", err: errors.New(`ent: missing required field "Day.date"`)}
	}
	if _, ok := dc.mutation.Txt(); !ok {
		return &ValidationError{Name: "txt", err: errors.New(`ent: missing required field "Day.txt"`)}
	}
	return nil
}
...
func (dc *DayCreate) createSpec() (*Day, *sqlgraph.CreateSpec) {
	...
	if value, ok := dc.mutation.Txt(); ok {
		_spec.Fields = append(_spec.Fields, &sqlgraph.FieldSpec{
	...
	if nodes := dc.mutation.PlanIDs(); len(nodes) > 0 {
